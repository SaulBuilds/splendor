...
| **S0.1** | Build on 3 platforms + CI | Splendor builds on Linux/Windows/macOS; a CI job produces artifacts; GPL source-publish step present. | A build with the retro shader syntactically broken **fails CI**, not silently skips. | ◐ **Linux/aarch64 VERIFIED** (builds + launches + EEVEE render smoke test passes; base Blender 5.3.0 Alpha; fixes in `df9d46b`). **Local CI**: `scripts/ci/run_tests.sh` + a pre-push git hook run all 8 `tests/splendor/` suites headlessly (ALL GREEN, ~4s) — no GitHub Actions (billing/binary). **Windows/macOS + hosted (self-hosted-runner) CI + the negative-control shader test still to do.** |
